...
import { emptyLandmarkMetrics } from "../lib/utils";
...
export function useCameraLandmarks(params: {
  ...
  function consumeLandmarkMetadata(includeTrackedSignals: boolean): Record<string, unknown> {
    ...
    if (pendingSignals.pitchValues.length > 0) {
      ...
      if (typeof pitch === "number") {
        ...
        metadata.pitch_ratio = Math.round((pitch / 180) * 10000) / 10000;
      }
    }
  ...
  function captureFrame(
    ...
    options?: { includeTrackedSignals?: boolean; extraMetadata?: Record<string, unknown> },
  ) {
    const video = videoRef.current;
